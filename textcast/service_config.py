...
def parse_interval(value: Union[str, int]) -> int:
    ...
    if isinstance(value, int):
        ...
        logger.warning(f"Using bare number {value} for interval is deprecated. Please use time units like '{value}m'")
        return value
    ...
    if isinstance(value, str):
        ...
        try:
            int(value.strip())
            raise ValueError(f"Bare numbers not allowed for intervals. Use '{value}m' instead of '{value}'")
    ...
    raise ValueError(f"Invalid interval format: {value}. Use format like '5m', '1h', '30s' with required time unit.")
...
class SourceConfig:
    ...
    file_patterns: List[str] = field(default_factory=lambda: ["*.mp3", "*.m4a", "*.wav"])
    # Common fields
...
def get_default_config_path() -> Path:
    ...
    if os.path.exists("/etc/nixos"):
        ...
        # macOS
        return home / "Library" / "Application Support" / "textcast-service" / "config.yaml"
