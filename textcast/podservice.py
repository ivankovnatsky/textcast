...
def upload_to_podservice(
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            logger.debug(
                f"File: {file_path} ({file_path.stat().st_size / 1024 / 1024:.1f} MB)"
            )
            ...
            if response.status_code == 201:
                ...
                try:
                    ...
                    logger.debug(
                        f"Episode URL: {result.get('episode', {}).get('audio_url', 'N/A')}"
                    )
                except Exception:
                    pass
                return True
                ...
                logger.info(
                    f"Episode already exists in podservice: {source_url or title}"
                )
                return True
                ...
                logger.warning(
                    f"Podservice upload failed with status {response.status_code}: {response.text}"
                )
            ...
            logger.warning(
                f"Podservice upload timed out after {timeout}s (attempt {attempt + 1}/{max_retries})"
            )
            ...
            logger.warning(
                f"Could not connect to podservice at {podservice_url} (attempt {attempt + 1}/{max_retries}): {e}"
            )
            last_error = str(e)
            continue
            ...
            logger.warning(
                f"Podservice upload error (attempt {attempt + 1}/{max_retries}): {e}"
            )
            last_error = str(e)
            continue
            ...
            logger.error(
                f"Unexpected error uploading to podservice: {e}", exc_info=True
            )
            return False

    # All retries exhausted
    logger.error(
        f"Failed to upload to podservice after {max_retries} attempts. Last error: {last_error}"
    )
    return False
